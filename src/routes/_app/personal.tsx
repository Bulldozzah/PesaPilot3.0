  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="Income" value={formatMoney(income)} accent="success" />
        <StatCard label="Expenses" value={formatMoney(expense)} accent="destructive" />
        <StatCard label="Net" value={formatMoney(income - expense)} accent={income - expense >= 0 ? "success" : "destructive"} />
      </div>
      <div className="mt-6 rounded-2xl border border-border bg-card divide-y">
        ...
        {rows.map((r) => (
          <div key={r.id} className="flex items-center justify-between p-4">
            ...
            <div className={`font-display text-lg font-semibold ${r.type === "income" ? "text-success" : "text-destructive"}`}>{r.type === "income" ? "+" : "−"} {formatMoney(r.amount)}</div>
          </div>
        ))}
      </div>
    </div>
