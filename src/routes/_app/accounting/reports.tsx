          ...
          <ChartCard title="P&L Summary">
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={chartData}>
                ...
                <XAxis dataKey="name" /><YAxis /><Tooltip formatter={(v: number) => formatMoney(v)} />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          ...
          <div className={`mt-2 rounded p-3 text-sm ${balanced ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
            {balanced ? "✓ Balance sheet balances" : `⚠ Out of balance by ${formatMoney(Math.abs(totalAssets - (totalLiab + totalEquity)))}`}
          </div>
        </div>
        <div className="space-y-6">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.a.id} className={`border-b border-border/50 ${abnormal(r) ? "bg-amber-500/10" : ""}`}>
                  ...
                  <td className="text-right">{r.debit ? formatMoney(r.debit) : ""}</td>
                  <td className="text-right">{r.credit ? formatMoney(r.credit) : ""}</td>
                </tr>
              ))}
              <tr className="font-bold"><td className="py-2" colSpan={2}>Totals</td><td className="text-right">{formatMoney(td)}</td><td className="text-right">{formatMoney(tc)}</td></tr>
            </tbody>
          </table>
          <div className={`mt-3 rounded p-3 text-sm ${Math.abs(td - tc) < 0.01 ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
            {Math.abs(td - tc) < 0.01 ? "✓ Trial balance is balanced" : `⚠ Out of balance by ${formatMoney(Math.abs(td - tc))}`}
          </div>
        </div>
        ...
        {data.map((g) => (
          <div key={g.a.id} className="rounded-lg border border-border p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <div className="text-sm">Closing: <span className="font-mono">{formatMoney(g.closing)}</span></div>
            </div>
            <div className="grid gap-4 lg:grid-cols-2">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {g.lines.map((l, i) => (
                    <tr key={i} className="border-b border-border/50"><td className="py-1">{l.date}</td><td>{l.desc}</td><td className="text-right">{l.debit ? formatMoney(l.debit) : ""}</td><td className="text-right">{l.credit ? formatMoney(l.credit) : ""}</td><td className="text-right font-mono">{formatMoney(l.balance)}</td></tr>
                  ))}
                </tbody>
              </table>
              {g.lines.length > 1 && (
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={g.lines.map((l) => ({ date: l.date, balance: l.balance }))}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="date" /><YAxis /><Tooltip formatter={(v: number) => formatMoney(v)} />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {e.journal_lines?.map((l) => (
                  <tr key={l.id} className="border-b border-border/50"><td className="py-1">{l.chart_of_accounts?.code} — {l.chart_of_accounts?.name}</td><td className="text-right">{l.debit ? formatMoney(Number(l.debit)) : ""}</td><td className="text-right">{l.credit ? formatMoney(Number(l.credit)) : ""}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
...
function ExpenseAnalysis({ accounts, startDate, endDate, balanceInRange }: RenderProps) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 rounded-lg bg-primary/5 p-3 text-sm">Total Revenue Reference: <span className="font-bold">{formatMoney(totalRev)}</span></div>
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          {cogs.length > 0 && (
            <>
              ...
              <table className="mb-4 w-full text-sm">
                ...
                <tbody>
                  {cogs.map((r) => <tr key={r.a.id} className="border-b border-border/50"><td className="py-1">{r.a.name}</td><td className="text-right">{formatMoney(r.v)}</td><td className="text-right">{pct(r.v)}</td></tr>)}
                </tbody>
              </table>
            </>
          )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {exp.map((r) => <tr key={r.a.id} className="border-b border-border/50"><td className="py-1">{r.a.name}</td><td className="text-right">{formatMoney(r.v)}</td><td className="text-right">{pct(r.v)}</td><td className="text-right">{totalExp > 0 ? `${((r.v / totalExp) * 100).toFixed(1)}%` : "—"}</td></tr>)}
              <tr className="font-bold"><td className="py-1">Total</td><td className="text-right">{formatMoney(totalExp)}</td><td className="text-right">{pct(totalExp)}</td><td></td></tr>
            </tbody>
          </table>
        </div>
        <div className="space-y-6">
          <ChartCard title="Expense Breakdown">
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={exp.map((r) => ({ name: r.a.name, value: r.v }))}>
                <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" hide /><YAxis /><Tooltip formatter={(v: number) => formatMoney(v)} />
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid gap-3 md:grid-cols-4">
        <StatCard label="Total Revenue" value={formatMoney(total)} />
        <StatCard label="Current Month" value={formatMoney(current)} />
        <StatCard label="MoM Growth" value={`${mom.toFixed(1)}%`} accent={mom >= 0 ? "success" : "destructive"} />
        <StatCard label="vs 12mo Avg" value={formatMoney(variance)} accent={variance >= 0 ? "success" : "destructive"} />
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <ChartCard title="12-Month Revenue Trend">
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={months}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="label" /><YAxis /><Tooltip formatter={(v: number) => formatMoney(v)} />
      ...
      <div className="mt-6">
        ...
        {bySource.map((r) => (
          <div key={r.name} className="mb-2">
            <div className="flex justify-between text-sm"><span>{r.name}</span><span>{formatMoney(r.value)} · {total > 0 ? ((r.value / total) * 100).toFixed(1) : 0}%</span></div>
...
function AgingReport({ accounts, entries, endDate, kind }: RenderProps & { kind: "ap" | "ar" }) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 rounded-lg bg-primary/5 p-3 text-sm">Total Outstanding: <span className="font-bold">{formatMoney(total)}</span></div>
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{bucketData.map((b) => <tr key={b.name} className="border-b border-border/50"><td className="py-1">{b.name}</td><td className="text-right">{formatMoney(b.value)}</td></tr>)}</tbody>
          </table>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{open.map((o, i) => <tr key={i} className="border-b border-border/50"><td className="py-1">{o.date}</td><td>{o.desc}</td><td className="text-right">{formatMoney(o.remaining)}</td></tr>)}</tbody>
          </table>
        </div>
...
function Row({ label, value, bold, accent, className }: { label: string; value: number; bold?: boolean; accent?: "success" | "destructive"; className?: string }) {
  ...
  return (
    <div className={`flex items-center justify-between border-b border-border/40 py-1.5 text-sm ${bold ? "font-bold" : ""} ${c} ${className ?? ""}`}>
      <span>{label}</span><span className="font-mono">{formatMoney(value)}</span>
    </div>
  );
}
