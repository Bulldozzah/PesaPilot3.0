...
import {
  Scale, FileText, List, BookOpen, Plus, Trash2, Check, Upload, X, Pencil,
} from "lucide-react";
...
function AccountingPage() {
  ...
  const reload = async () => {
    ...
    const [{ data: a }, { data: s }, { data: contacts }, { data: e }] = await Promise.all([
      ...
      supabase
        .from("contacts")
        .select("id,name,type,user_business_id")
        .or(`user_business_id.eq.${businessId},user_business_id.is.null`)
        .order("name"),
      supabase
    ...
    const contactList = (contacts as { id: string; name: string; type: string }[]) ?? [];
    setVendors(contactList.filter((x) => x.type === "vendor").map((x) => ({ id: x.id, name: x.name })));
    setCustomers(contactList.filter((x) => x.type === "customer").map((x) => ({ id: x.id, name: x.name })));
    ...
    <div className="rounded-2xl border border-border bg-card">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.account.id} className="border-b border-border/50">
              ...
              <td className="text-right">{r.displayDebit ? money(r.displayDebit) : ""}</td>
              <td className="pr-4 text-right">{r.displayCredit ? money(r.displayCredit) : ""}</td>
            </tr>
          ))}
          <tr className="bg-muted/50 font-bold">
            <td className="py-2 pl-4" colSpan={3}>Totals</td>
            <td className="text-right">{money(totalDebit)}</td>
            <td className="pr-4 text-right">{money(totalCredit)}</td>
          </tr>
        </tbody>
      </table>
      <div className="p-4 text-center text-sm">
        {balanced ? (
          ...
          <span className="text-destructive font-medium">⚠️ Out of balance by {money(Math.abs(totalDebit - totalCredit))}</span>
        )}
      </div>
    </div>
  );
...
function JournalEntries({
  ...
  const [editingEntry, setEditingEntry] = useState<JEntry | null>(null);
  ...
  const reset = () => {
    ...
    setRef(""); setDesc(""); setLines([emptyLine(), emptyLine()]); setShowForm(false); setEditingEntry(null);
  };

  const editEntry = (e: JEntry) => {
    setEditingEntry(e);
    setDate(e.entry_date);
    setRef(e.reference ?? "");
    setDesc(e.description ?? "");
    setLines(e.journal_lines?.map((l) => ({
  ...
  const save = async (post: boolean) => {
    ...
    if (editingEntry) {
      ...
      const reference = ref.trim() || editingEntry.reference || await autoRef();
      const { error } = await supabase
        .from("journal_entries")
        .update({ entry_date: date, reference, description: desc, is_posted: post })
        .eq("id", editingEntry.id);
      if (error) { toast.error(error.message); return; }

      // Delete old lines and insert new ones
      await supabase.from("journal_lines").delete().eq("journal_entry_id", editingEntry.id);
      const payload = lines.map((l) => ({
        journal_entry_id: editingEntry.id, account_id: l.account_id,
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        <Button onClick={() => { if (showForm) reset(); else setShowForm(true); }}><Plus className="mr-2 h-4 w-4" />{showForm ? "Cancel" : "New Entry"}</Button>
      </div>
        ...
        <div className="space-y-4 rounded-2xl border border-border bg-card p-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-muted/40 px-4 py-3 text-sm">
            ...
            <div className="flex items-center gap-4">
              <span>Totals: <b>{money(totalDebit)}</b> | <b>{money(totalCredit)}</b></span>
              {balanced ? (
                <span className="text-success font-medium">✓ Balanced</span>
              ) : totalDebit || totalCredit ? (
                <span className="text-destructive font-medium">⚠️ Out of balance: {money(Math.abs(totalDebit - totalCredit))}</span>
              ) : null}
            </div>
          </div>

          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => save(false)}>{editingEntry ? "Update as Draft" : "Save as Draft"}</Button>
            <Button onClick={() => save(true)} disabled={!balanced}>{editingEntry ? "Update & Post" : "Save & Post"}</Button>
          </div>
        </div>
      )}
          ...
          <div key={e.id} className="rounded-2xl border border-border bg-card">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 p-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {!e.is_posted && <Button size="icon" variant="ghost" onClick={() => editEntry(e)}><Pencil className="h-4 w-4" /></Button>}
              ...
              <tbody>
                {e.journal_lines?.map((l) => (
                  <tr key={l.id} className="border-t border-border/30">
                    ...
                    <td className="text-right">{Number(l.debit) > 0 ? money(l.debit) : ""}</td>
                    <td className="pr-4 text-right">{Number(l.credit) > 0 ? money(l.credit) : ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <tbody>
            ...
            {ledger.map((r, i) => (
              <tr key={i} className="border-b border-border/40">
                ...
                <td>{r.desc}</td>
                <td className="text-right">{r.debit > 0 ? <span className="text-success">↗ {money(r.debit)}</span> : ""}</td>
                <td className="text-right">{r.credit > 0 ? <span className="text-destructive">↘ {money(r.credit)}</span> : ""}</td>
                <td className="pr-4 text-right font-medium">{money(r.balance)}</td>
              </tr>
            ))}
            {ledger.length > 0 && (
              <tr className="bg-muted/40 font-bold">
                <td className="py-2 pl-4" colSpan={3}>Ending Balance</td>
                <td className="text-right">{money(ledger.reduce((s, r) => s + r.debit, 0))}</td>
                <td className="text-right">{money(ledger.reduce((s, r) => s + r.credit, 0))}</td>
                <td className="pr-4 text-right">{money(ledger[ledger.length - 1]?.balance ?? 0)}</td>
              </tr>
            )}
          </tbody>
        </table>
                ...
                {list.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="flex items-center justify-between border-b border-border/30 px-4 py-2 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm">{money(Math.abs(bal))} {credit && bal !== 0 ? <span className="text-xs text-muted-foreground">CR</span> : null}</span>
